...
namespace HXSL
{
	...
	private:
		...
		Primitive(const TextSpan& span, IdentifierInfo* name, PrimitiveKind kind, PrimitiveClass _class, uint16_t rows, uint16_t columns)
	...
	public:
		...
		static Primitive* Create(const TextSpan& span, IdentifierInfo* name, PrimitiveKind kind, PrimitiveClass _class, uint16_t rows, uint16_t columns, Span<OperatorOverload*>& operators);
		static Primitive* Create(const TextSpan& span, IdentifierInfo* name, PrimitiveKind kind, PrimitiveClass _class, uint16_t rows, uint16_t columns, uint32_t numOperators);

		DEFINE_TRAILING_OBJ_SPAN_GETTER(GetOperators, 0, storage);

		uint16_t GetRows() const noexcept { return rows; }
		void SetRows(const uint16_t& value) noexcept { rows = value; }

		uint16_t GetColumns() const noexcept { return columns; }
		void SetColumns(const uint16_t& value) noexcept { columns = value; }
