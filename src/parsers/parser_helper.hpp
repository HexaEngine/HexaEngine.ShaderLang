...
namespace HXSL
{
	...
	public:
		...
		static bool ParseFunctionCallInner(const Token& start, LazySymbol& lazy, Parser& parser, TokenStream& stream, std::unique_ptr<FunctionCallExpression>& expression);

		static bool TryParseFunctionCall(Parser& parser, TokenStream& stream, std::unique_ptr<FunctionCallExpression>& expression);

		static bool TryParseSymbol(Parser& parser, TokenStream& stream, std::unique_ptr<Expression>& expressionOut);

		static bool TryParseLiteralExpression(Parser& parser, TokenStream& stream, std::unique_ptr<LiteralExpression>& expressionOut);

		static bool TryParseInitializationExpression(Parser& parser, TokenStream& stream, std::unique_ptr<InitializationExpression>& expressionOut);
