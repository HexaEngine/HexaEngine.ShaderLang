...
namespace HXSL
{
	...
	{
	public:
		virtual void HandleExpression(SemanticAnalyzer& analyzer, TypeChecker& checker, SymbolResolver& resolver, Expression* node, std::stack<Expression*>& stack) = 0;
	};

	template<class T>
	class ExpressionChecker : public ExpressionCheckerBase
	{
		void HandleExpression(SemanticAnalyzer& analyzer, TypeChecker& checker, SymbolResolver& resolver, Expression* node, std::stack<Expression*>& stack)
		{
			T* expression = static_cast<T*>(node);
			HandleExpression(analyzer, checker, resolver, expression, stack);
		}

		virtual void HandleExpression(SemanticAnalyzer& analyzer, TypeChecker& checker, SymbolResolver& resolver, T* expression, std::stack<Expression*>& stack) = 0;
	};
	...
	{
		void HandleExpression(SemanticAnalyzer& analyzer, TypeChecker& checker, SymbolResolver& resolver, Expression* node, std::stack<Expression*>& stack) override
		{
		}
	};
	...
	{
		void HandleExpression(SemanticAnalyzer& analyzer, TypeChecker& checker, SymbolResolver& resolver, MemberAccessExpression* expression, std::stack<Expression*>& stack);
	};

	class MemberReferenceExpressionChecker : public ExpressionChecker<MemberReferenceExpression>
	{
		void HandleExpression(SemanticAnalyzer& analyzer, TypeChecker& checker, SymbolResolver& resolver, MemberReferenceExpression* expression, std::stack<Expression*>& stack);
	};
	...
	{
		void HandleExpression(SemanticAnalyzer& analyzer, TypeChecker& checker, SymbolResolver& resolver, LiteralExpression* expression, std::stack<Expression*>& stack);
	};
	...
	{
		void HandleExpression(SemanticAnalyzer& analyzer, TypeChecker& checker, SymbolResolver& resolver, IndexerAccessExpression* expression, std::stack<Expression*>& stack);
	};

	class AssignmentChecker : public ExpressionChecker<AssignmentExpression>
	{
		void HandleExpression(SemanticAnalyzer& analyzer, TypeChecker& checker, SymbolResolver& resolver, AssignmentExpression* expression, std::stack<Expression*>& stack);
	};

	class CompoundAssignmentChecker : public ExpressionChecker<CompoundAssignmentExpression>
	{
		void HandleExpression(SemanticAnalyzer& analyzer, TypeChecker& checker, SymbolResolver& resolver, CompoundAssignmentExpression* expression, std::stack<Expression*>& stack);
	};
}
